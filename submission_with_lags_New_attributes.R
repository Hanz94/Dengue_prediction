...
Time_Series_Lags_for_sj <- function(sj_train_set)
{
  ...
  sj_train_set$station_max_temp_c %<>% lag(., 6) 
  sj_train_set$reanalysis_max_air_temp_k %<>% lag(., 6) 
  sj_train_set$reanalysis_avg_temp_k %<>% lag(., 7)
...
Time_Series_Lags_for_iq <- function(iq_train_set)
{
  ...
  iq_train_set$reanalysis_precip_amt_kg_per_m2 %<>% lag(., 2) 
...
write.csv(predictions, 'G:/S7/Data Mining/Dengue/Submissions/predictions_with_lags_new_features_2.csv', row.names = FALSE)
